...
def _fallback_cold_start_performance(
    ...
    cold_default = (
        float(cold_df["did_default"].mean() * 100.0) if not cold_df.empty else 0.0
    )
    warm_default = (
        float(warm_df["did_default"].mean() * 100.0) if not warm_df.empty else 0.0
    )
    improvement = (
        ((warm_reward - cold_reward) / abs(cold_reward) * 100.0) if cold_reward else 0.0
    )
...
def _fallback_shock_recovery(
    results_df: pd.DataFrame, policy: str, shock_month: int = 6
) -> dict:
    if "policy" in results_df.columns:
        policy_df = results_df.loc[results_df["policy"] == policy]
    else:
...
def _fallback_build_full_metrics_table(
    ...
    oracle_total = (
        float(oracle_df["reward_received"].sum())
    ...
    for policy in sorted(results_df["policy"].unique()):
        ...
        regret_vs_oracle_pct = (
            float(((oracle_total - total_revenue) / oracle_total) * 100.0)
            if oracle_total
            else 0.0
        )
        ...
        if policy in {
            ...
            exploration = compute_exploration_ratio_dict(results_df, policy).get(
                "exploration_ratio", 0.0
            )
...
compute_convergence_month = getattr(
    evaluate_metrics, "compute_convergence_month", lambda results_df, policy: 12
)
...
def dark_chart(fig, title=None, height=320):
    existing_title = (
        fig.layout.title.text if fig.layout.title and fig.layout.title.text else None
    )
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            ...
            linecolor="#1e2a45",
            tickcolor="#1e2a45",
            tickfont=dict(size=10, color="#93a1b9"),
            zeroline=False,
        ),
        yaxis=dict(
            gridcolor="#1e2a45",
...
def require_data(results_df: pd.DataFrame, users_df: pd.DataFrame) -> bool:
    if results_df.empty or users_df.empty:
        st.warning(
            "Missing `data/simulation_results.csv` or `data/synthetic_users.csv`. Generate them first."
        )
        return False
    return True
...
def monthly_cumulative_rewards(results_df: pd.DataFrame) -> pd.DataFrame:
    monthly = (
        results_df.groupby(["policy_display", "month"], as_index=False)[
            "reward_received"
        ]
        .sum()
        .sort_values(["policy_display", "month"])
    )
    monthly["cumulative_reward"] = monthly.groupby("policy_display")[
        "reward_received"
    ].cumsum()
    return monthly
...
def default_rate_by_month(results_df: pd.DataFrame) -> pd.DataFrame:
    return results_df.groupby(["policy_display", "month"], as_index=False)[
        "did_default"
    ].mean()
...
def build_custom_portfolio(
    users_df: pd.DataFrame, risk_distribution: dict[str, int], seed: int = 42
) -> pd.DataFrame:
    ...
    for tier, count in desired_counts.items():
        ...
        sampled = tier_df.sample(
            n=count, replace=True, random_state=int(rng.integers(0, 1_000_000))
        )
...
def apply_fraud_spike(
    outcomes: pd.DataFrame, users_df: pd.DataFrame, month: int, rng: np.random.Generator
) -> pd.DataFrame:
    if month != 9:
        return outcomes
    shocked = outcomes.merge(
        users_df[["user_id", "risk_tier"]], on="user_id", how="left"
    )
    ...
    shocked.loc[flip_idx, "outstanding_amount"] = np.round(
        shocked.loc[flip_idx, "amount_spent"]
        * rng.uniform(0.72, 1.0, size=len(flip_idx)),
        2,
    )
...
def run_live_thompson_simulation(
    ...
    current_limits = {
        row.user_id: float(row.initial_credit_limit)
        for row in live_users.itertuples(index=False)
    }
    ...
    for month in range(1, n_months + 1):
        ...
        for user_id in live_users["user_id"]:
            ...
            current_limits[user_id] = ContextBuilder.apply_action(
                current_limits[user_id], action
            )
        ...
        simulation_input["initial_credit_limit"] = simulation_input["user_id"].map(
            current_limits
        )
        ...
        for rr in ready_rewards:
            ...
            logs[row_lookup[(rr["user_id"], month)]]["reward_received"] = float(
                rr["reward"]
            )
        progress_bar.progress(
            month / n_months, text=f"Simulating month {month}/{n_months}..."
        )
...
def render_portfolio_overview(results_df: pd.DataFrame, users_df: pd.DataFrame) -> None:
    ...
    revenue_lift = (
        ((total_revenue - static_revenue) / static_revenue * 100.0)
        if static_revenue
        else 0.0
    )
    default_rate = float(thompson_df["did_default"].mean() * 100.0)
    regret_pct = (
        ((oracle_revenue - total_revenue) / oracle_revenue * 100.0)
        if oracle_revenue
        else 0.0
    )
    ...
    for m, imp in (
        rolling.pct_change().replace([np.inf, -np.inf], np.nan).dropna().items()
    ):
        if imp < 0.01:
            conv_month = int(m)
            break
    ...
    c1.metric("Revenue", format_inr(total_revenue), f"+{revenue_lift:.1f}% vs static")
    c2.metric(
        "Defaults",
        f"{default_rate:.2f}%",
    ...
    c4.metric("Convergence", f"Month {conv_month}", f"{conv_month}mo learning")
    ...
    with col2:
        col2.metric(
            ...
            delta_color=(
                "normal" if 10 <= exp_dict["exploration_ratio"] <= 25 else "inverse"
            ),
        )
    with col3:
        months = (
            shock["months_to_recovery"]
            if shock["months_to_recovery"] is not None
            else "N/A"
        )
        col3.metric(
            "Shock recovery",
            (
                f"{months} month{'s' if isinstance(months, int) and months != 1 else ''}"
                if isinstance(months, int)
                else "N/A"
            ),
            delta=(
                f"Peak default: {shock['peak_default_rate']:.1f}%"
                if shock["peak_default_rate"] > 0
                else "No shock impact"
            ),
        )
    with col4:
    ...
    # Tabs
    tab_rev, tab_def, tab_act, tab_tier, tab_cohort = st.tabs(
        ["Revenue", "Default Rate", "Actions Taken", "By Risk Tier", "Cohort Analysis"]
    )
    ...
    with tab_rev:
        ...
        fig.add_vline(
            ...
            line_color="#ef4444",
            line_width=1,
            annotation_text="Economic shock",
    ...
    with tab_act:
        action_monthly = (
            thompson_df.groupby(["month", "action_taken"])
            .size()
        ...
        dark_chart(
            fig3, title="Thompson Sampling - Action Distribution (%)", height=360
        )
    ...
    with tab_tier:
        ...
        for i, tier in enumerate(tier_names):
            ...
            t_def = (
                float(tier_data["did_default"].mean() * 100)
    ...
    with tab_cohort:
        ...
        if cohort_path.exists():
            ...
            pivot = cohort_df.pivot(
                index="risk_tier", columns="income_bucket", values="avg_reward_per_user"
            )
            ...
            if cohort2_path.exists():
                ...
                thompson_static = cohort2_df[
                    cohort2_df["policy"].isin(["thompson_sampling", "static_baseline"])
                ]

                tier_pivot = thompson_static.pivot(
                    index="risk_tier", columns="policy", values="total_revenue"
                )
                tier_pivot["lift_pct"] = (
                    (tier_pivot["thompson_sampling"] - tier_pivot["static_baseline"])
                    / tier_pivot["static_baseline"].clip(lower=1)
                    * 100
                )
                tier_pivot["thompson_formatted"] = tier_pivot[
                    "thompson_sampling"
                ].apply(format_inr)
                tier_pivot["static_formatted"] = tier_pivot["static_baseline"].apply(
                    format_inr
                )

                st.subheader("Revenue Impact by Risk Tier", divider="blue")
                display_cols = tier_pivot[
                    ["thompson_formatted", "static_formatted", "lift_pct"]
                ].copy()
            ...
            st.info(
                "Cohort analysis data not available. Run the generation notebook first."
            )
...
def render_user_deep_dive(results_df: pd.DataFrame, users_df: pd.DataFrame) -> None:
    ...
    selected_input = sel_col.text_input(
        "User ID",
        value=st.session_state.selected_user_id,
        placeholder="e.g. USER_00042",
    )
    if btn_col.button("🎲 Random User"):
        st.session_state.selected_user_id = str(
            np.random.default_rng().choice(user_ids)
        )
        st.rerun()
    ...
    if user_row.empty:
        st.markdown(
            f"""<div style="background:#161b2e;border:1px dashed #1e2a45;border-radius:10px;
    ...
    if user_results.empty:
        st.markdown(
            """<div style="background:#161b2e;border:1px dashed #1e2a45;border-radius:10px;
    ...
    thompson_user = user_results[
        user_results["policy"] == "thompson_sampling"
    ].sort_values("month")
    ...
    rt_bg = {
        ...
        "Near-Prime": "#0d1a2d",
        "Subprime": "#2a1d0a",
        "Deep-Subprime": "#2a0d0d",
    }.get(risk_tier, "#1a1f35")
    ...
    acct_age = (
        int(user_row["account_age_months"])
        if "account_age_months" in user_row.index
        else "N/A"
    )
    init_limit = (
        float(user_row["initial_credit_limit"])
        if "initial_credit_limit" in user_row.index
        else 0
    )
    ...
    with left:
        ...
        fig_limit.add_trace(
            go.Scatter(
                x=thompson_user["month"],
                y=thompson_user["current_limit"],
                mode="lines+markers",
                line=dict(color="#3b82f6", width=2),
        ...
        if not actions_taken.empty:
            fig_limit.add_trace(
                go.Scatter(
                    x=actions_taken["month"],
                    y=actions_taken["current_limit"],
                    mode="markers",
                    marker=dict(
                        size=8,
                        color=[
                            ACTION_COLORS.get(a, "#888")
                            for a in actions_taken["action_taken"]
                        ],
                    ),
                    name="Increase",
                    showlegend=True,
    ...
    with right:
        ...
        fig_rew.add_trace(
            go.Bar(
                x=thompson_user["month"],
                y=thompson_user["reward_received"],
                marker_color=colors,
                name="Reward",
            )
        )
    ...
    with st.expander("How did the bandit make decisions for this user?"):
        ...
        if not user_data.empty:
            ...
            cibil = (
                int(user_data["cibil_score"].values[0])
                if "cibil_score" in user_data.columns
                else "Unknown"
            )
...
def render_policy_comparison(results_df: pd.DataFrame, users_df: pd.DataFrame) -> None:
    ...
    winner = max(
        (k for k in policy_revenues if k not in ["Static Baseline", "Oracle"]),
        key=lambda k: policy_revenues[k],
    ...
    tab_reward, tab_regret, tab_summary = st.tabs(
        ["Reward Curves", "Regret Analysis", "Summary Table"]
    )
    ...
    with tab_reward:
        ...
        t_monthly = (
            results_df[results_df["policy"] == "thompson_sampling"]
            .groupby("month")["reward_received"]
            .sum()
        )
        s_monthly = (
            results_df[results_df["policy"] == "static_baseline"]
            .groupby("month")["reward_received"]
            .sum()
        )
        delta_df = pd.DataFrame(
            {"month": t_monthly.index, "delta": (t_monthly - s_monthly).values}
        )
        ...
        dark_chart(
            fig_delta, title="Monthly Revenue Delta (Thompson - Static)", height=280
        )
    ...
    with tab_regret:
        ...
        for policy_key, label in [
            ...
            regret_col = (
                "regret_cumulative"
                if "regret_cumulative" in regret_df.columns
                else "regret"
            )
    ...
    with tab_summary:
        ...
        def render_metrics_table(df):
            ...
            for _, row in df.iterrows():
                ...
                convergence = (
                    f"{row['convergence_month']}"
                    if isinstance(row["convergence_month"], int)
                    else str(row["convergence_month"])
                )
                exploration = (
                    f"{row['exploration_pct']:.1f}%"
                    if isinstance(row["exploration_pct"], float)
                    else str(row["exploration_pct"])
                )
                ...
                """
                rows_html += (
                    f"<tr style='border-bottom:0.5px solid #1e2a45'>{cells}</tr>"
                )
...
def render_live_simulation(users_df: pd.DataFrame, results_df: pd.DataFrame) -> None:
    page_header(
        "Live Simulation", "Adjust parameters and re-run Thompson Sampling in real time"
    )
    ...
    with left_col:
        ...
        fraud_spike_toggle = st.toggle(
            "Fraud spike at month 9", help="Doubles Deep-Subprime defaults at month 9"
        )

        st.markdown(
            """<div style="font-size:12px;font-weight:500;color:#c9d1e0;margin:12px 0 4px">
            Risk Distribution (must sum to 100%)</div>""",
            unsafe_allow_html=True,
        )
        ...
        n_months = st.select_slider(
            "Simulation Horizon (months)", options=[6, 9, 12], value=12
        )
        run_btn = st.button("Run Simulation", width="stretch")

    risk_distribution = {
        "Prime": prime_pct,
        "Near-Prime": near_prime_pct,
        "Subprime": subprime_pct,
        "Deep-Subprime": max(deep_sub_pct, 0),
    }
    ...
    with right_col:
        ...
        if live_results is None:
            ...
            static_df = results_df.loc[results_df["policy"] == "static_baseline"]
            static_total = (
                float(static_df["reward_received"].sum())
                if not static_df.empty
                else 0.0
            )
            live_total = float(live_results["reward_received"].sum())
            rev_lift = (
                ((live_total - static_total) / static_total * 100)
                if static_total
                else 0
            )
            ...
            for m, imp in (
                roll.pct_change().replace([np.inf, -np.inf], np.nan).dropna().items()
            ):
                if imp < 0.01:
                    conv = int(m)
                    break
            ...
            # Cumulative chart
            monthly = live_results.groupby("month", as_index=False)[
                "reward_received"
            ].sum()
            ...
            d1, d2 = st.columns(2)
            with d1:
                def_monthly = live_results.groupby("month", as_index=False)[
                    "did_default"
                ].mean()
                ...
                fig_d.add_hline(
                    y=4.0, line_dash="dash", line_color="#4a5568", line_width=1
                )
            ...
            with d2:
                act_m = (
                    live_results.groupby(["month", "action_taken"])
                    .size()
                    .reset_index(name="count")
                )
...
def render_sidebar():
    with st.sidebar:
        ...
        st.markdown(
            """<div style="font-size:10px;color:#8f9bb1;text-transform:uppercase;
...
def main() -> None:
    ...
    if selected_page in {
        ...
        "Per-User Deep Dive",
        "Policy Comparison",
    } and not require_data(results_df, users_df):
        return
