...
def _with_user_features(results_df: pd.DataFrame) -> pd.DataFrame:
    if {"risk_tier", "income_bucket", "account_age_months"}.issubset(
        results_df.columns
    ):
...
def _monthly_rewards(
    frame: pd.DataFrame, reward_column: str = "reward_received"
) -> pd.Series:
...
def _total_reward(frame: pd.DataFrame) -> float:
    reward_column = (
        "reward_received" if "reward_received" in frame.columns else "reward"
    )
...
def compute_cumulative_regret(oracle_df, policy_df) -> pd.DataFrame:
    ...
    if "month" not in oracle_df.columns or "month" not in policy_df.columns:
        raise ValueError(
            "Cumulative regret requires month-level results with a 'month' column"
        )

    reward_column_oracle = (
        "reward_received" if "reward_received" in oracle_df.columns else "reward"
    )
    reward_column_policy = (
        "reward_received" if "reward_received" in policy_df.columns else "reward"
    )

    oracle_by_month = (
        oracle_df.groupby("month")[reward_column_oracle]
        .sum()
        .cumsum()
        .reset_index()
    ...
    policy_by_month = (
        policy_df.groupby("month")[reward_column_policy]
        .sum()
        .cumsum()
        .reset_index()
    ...
    merged["regret_cumulative"] = (
        merged["oracle_cumulative"] - merged["policy_cumulative"]
    )
    merged["regret_pct"] = (
        merged["regret_cumulative"] / merged["oracle_cumulative"] * 100
    ).clip(0, 100)
    return merged[
        [
            "month",
...
def cohort_analysis(results_df, groupby: list[str]) -> pd.DataFrame:
    ...
    cohort_df = (
        ...
        .agg(
            ...
            avg_reward_per_user=(
                "reward_received",
                lambda s: s.sum() / max(s.index.nunique(), 1),
            ),
    ...
    cohort_df["avg_reward_per_user"] = cohort_df["total_reward"] / cohort_df[
        "n_users"
    ].clip(lower=1)
    return cohort_df
...
def shock_recovery_analysis(
    pre_shock_df, post_shock_df, n_months_post: int = 6
) -> dict:
    ...
    post_default_by_month = (
        post_shock_df.groupby("month")["did_default"]
        .mean()
        .sort_index()
        .head(n_months_post)
    )
    post_reward_by_month = (
        post_shock_df.groupby("month")["reward_received"]
        .sum()
        .sort_index()
        .head(n_months_post)
    )

    pre_reward_baseline = float(
        pre_shock_df.groupby("month")["reward_received"].sum().mean()
    )
    pre_reward_baseline = pre_reward_baseline if pre_reward_baseline != 0 else 1.0

    max_default_spike_pct = float(
        max(((post_default_by_month - pre_default_rate) * 100.0).max(), 0.0)
        if not post_default_by_month.empty
        else 0.0
    )
    ...
    reward_recovery_pct = (
        float(post_reward_by_month.iloc[-1] / pre_reward_baseline * 100.0)
        if not post_reward_by_month.empty
        else 0.0
    )
    return {
...
def _convergence_month(results_df: pd.DataFrame) -> int:
    ...
    rolling_improvement_pct = rolling_rewards.pct_change().replace(
...
def policy_comparison_table(all_results: dict[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    for policy_name, frame in all_results.items():
        ...
        regret_vs_oracle_pct = float(
            ((oracle_total_reward - total_reward) / oracle_total_reward) * 100.0
            if oracle_total_reward
            else 0.0
        )
        revenue_lift_vs_static_pct = float(
            ((total_reward - static_total_reward) / static_total_reward) * 100.0
            if static_total_reward
            else 0.0
        )
...
def compute_cold_start_performance(
    ...
    cold_start_avg_reward = (
        float(cold_start_df["reward_received"].mean())
        if not cold_start_df.empty
        else 0.0
    )
    warm_avg_reward = (
        float(warm_df["reward_received"].mean()) if not warm_df.empty else 0.0
    )

    cold_start_default_rate = (
        float(cold_start_df["did_default"].mean() * 100.0)
        if not cold_start_df.empty
        else 0.0
    )
    warm_default_rate = (
    ...
    if cold_start_avg_reward != 0:
        reward_improvement_pct = (
            (warm_avg_reward - cold_start_avg_reward) / abs(cold_start_avg_reward)
        ) * 100.0
    else:
        reward_improvement_pct = 0.0
...
def compute_shock_recovery(
    ...
    pre_shock_default_rate = (
        float(pre_shock_df["did_default"].mean()) if not pre_shock_df.empty else 0.0
    )
    ...
    post_default_by_month = (
        post_shock_df.groupby("month")["did_default"].mean().sort_index()
    )
    ...
    max_spike_pct = float(
        (
            (peak_default_rate - pre_shock_default_rate)
            / max(pre_shock_default_rate, 0.001)
        )
        * 100.0
    )
...
def build_full_metrics_table(
    results_df: pd.DataFrame, users_df: pd.DataFrame, oracle_df: pd.DataFrame = None
) -> pd.DataFrame:
    """
    ...
    for policy in policies:
        ...
        regret_vs_oracle_pct = float(
            ((oracle_total_reward - total_revenue_inr) / oracle_total_reward) * 100.0
        )
